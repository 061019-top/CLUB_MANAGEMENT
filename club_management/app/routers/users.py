...
@router.get("/me" , summary="Xem hồ sơ cá nhân", response_model=ApiResponse[UserResponse], description='Xem hồ sơ của tài khoản trong HTTP Bearer token.')
...
@router.get("", summary="Danh sách người dùng", response_model=ApiResponse[Page[UserResponse]], description='Chỉ ADMIN được tìm kiếm theo tên/email, lọc trạng thái và phân trang người dùng.')
def read_users(
    request: Request,
    ...
    limit: int = Query(20, ge=1, le=50, description="Số lượng kết quả lấy ra (Tối đa 50)"),
